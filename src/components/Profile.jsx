...
export default function Profile({ name = 'Anshu', email = 'anshu@test.com' }){
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      >
        <div className="w-7 h-7 rounded-full bg-amber-600 text-white flex items-center justify-center text-xs font-bold">{name ? name[0] : 'U'}</div>
        <div className="text-sm text-gray-200 hidden sm:block">{name}</div>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-64 bg-gray-900 border border-gray-800 rounded-lg p-3 shadow-lg">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 rounded-full bg-sky-500 text-white flex items-center justify-center font-bold">{name ? name[0] : 'U'}</div>
          ...
          <div className="mt-3 border-t border-gray-800 pt-2 space-y-2">
            ...
            <button
              type="button"
              onClick={() => { /* TODO: sign out */ alert('Sign out') }}
