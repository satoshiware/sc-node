		...
		<div className="min-h-screen p-4">
			<div className="sticky top-0 z-30 bg-gray-900/60 backdrop-blur-sm px-3 py-2 rounded-b-md mb-4 flex items-center justify-between">
				...
				<div className="flex items-center gap-6">
					...
					<div className="text-xl font-semibold">${price.toLocaleString()} <span className={change>=0 ? 'text-green-400 text-sm' : 'text-red-400 text-sm'}>{change>=0? `+${change}`: change}</span></div>
				</div>
			</div>
			...
			<div className="mt-4 grid grid-cols-12 gap-4">
				<div className="col-span-8 space-y-4">
					...
					<div className="bg-gray-800 p-3 rounded-md">
						...
						<div className="text-xs text-gray-400 grid grid-cols-6 gap-2 border-b border-gray-700 pb-2">
							<div>Time</div>
							<div>Type</div>
							<div>Side</div>
							<div>Price</div>
					...
					<div className="grid grid-cols-2 gap-3">
						<div className="col-span-1">
							<div className="bg-gray-800 p-3 rounded-md">
								...
								<div className="flex gap-2">
									<div className="flex-1 max-h-56 overflow-y-auto pr-2">
										{orderbook.bids.map((b,i)=> (
											<div key={i} className="flex items-center justify-between text-sm text-green-300 py-1">{b.size} <span className="text-gray-300">{b.price}</span></div>
										))}
									</div>
									<div className="flex-1 max-h-56 overflow-y-auto border-l border-gray-700 pl-2">
										{orderbook.asks.map((a,i)=> (
											<div key={i} className="flex items-center justify-between text-sm text-red-300 py-1"><span className="text-gray-300">{a.price}</span> {a.size}</div>
										))}
									</div>
								</div>
							</div>
						...
						<div className="col-span-1">
							<div className="bg-gray-800 p-3 rounded-md">
								...
								<div className="space-y-2 text-sm text-gray-300 max-h-56 overflow-y-auto pr-2">
									{trades.length === 0 ? (
										...
										<div key={i} className="flex items-center justify-between border-b border-gray-800 pb-1">
											...
											<div className="text-sm">{t.amount} @ {t.price}</div>
										</div>
									))}
								</div>
							</div>
