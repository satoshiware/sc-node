...
def load_sqlite_settlement_context(session: Session, settlement_id: int) -> SQLiteSettlementContext | None:
    ...
    identity_deltas = compute_identity_share_deltas(session, work_window_start, work_window_end)
...
def uses_work_basis_for_shadow_write(context: SQLiteSettlementContext) -> bool:
    ...
    contributions_by_username = context.user_contributions
    for payout, user in context.payout_rows:
        contribution_value = Decimal(str(payout.contribution_value or 0))
        share_delta = Decimal(str((contributions_by_username.get(user.username) or UserContribution(user.username, 0, Decimal("0"))).share_delta))
...
def sqlite_summary(context: SQLiteSettlementContext) -> dict[str, object]:
    ...
    return {
        ...
        "total_reward_sats": btc_to_sats(context.settlement.pool_reward_btc),
        "total_work": to_decimal_str(normalize_work(context.settlement.total_work)),
...
def postgres_summary(
    ...
    context: SQLiteSettlementContext,
    ...
    return {
        ...
        "total_work": to_decimal_str(normalize_work(settlement_row["total_work"] or 0)),
...
def compare_postgres_shadow_settlement(
    ...
    *,
    ...
    context = load_sqlite_settlement_context(session, settlement_id)
    if context is None:
        return (
            {
                "status": "ok",
    ...
    sqlite_payload = sqlite_summary(context)

    try:
        effective_repository = repository or get_postgres_shadow_compare_repository()
        postgres_payload = postgres_summary(effective_repository, context)
    ...
    if int(sqlite_payload["total_reward_sats"]) != int(postgres_payload["total_reward_sats"]):
        mismatches.append(
            _mismatch(
                "total_reward_sats",
                sqlite_payload["total_reward_sats"],
                postgres_payload["total_reward_sats"],
                "SQLite reward total does not match Postgres settlement reward total.",
            )
        )
    if sqlite_payload["total_work"] != postgres_payload["total_work"]:
        mismatches.append(
            _mismatch(
                "total_work",
                sqlite_payload["total_work"],
                postgres_payload["total_work"],
                "SQLite total_work does not match Postgres total_work after SQLite precision normalization.",
            )
        )
    if int(sqlite_payload["total_shares"]) != int(postgres_payload["total_shares"]):
        mismatches.append(
            _mismatch(
                "total_shares",
                sqlite_payload["total_shares"],
                postgres_payload["total_shares"],
                "SQLite total_shares does not match Postgres total_shares.",
            )
        )
    if int(sqlite_payload["user_payout_count"]) != int(postgres_payload["user_payout_count"]):
        mismatches.append(
            _mismatch(
                "user_payout_count",
                sqlite_payload["user_payout_count"],
                postgres_payload["user_payout_count"],
                "SQLite payout row count does not match Postgres settlement_user_credits count.",
            )
        )
    if int(sqlite_payload["rewarded_block_count"]) != int(postgres_payload["rewarded_block_count"]):
        mismatches.append(
            _mismatch(
                "rewarded_block_count",
                sqlite_payload["rewarded_block_count"],
                postgres_payload["rewarded_block_count"],
                "SQLite rewarded block count does not match Postgres settlement block count.",
            )
        )
    if int(sqlite_payload["rewarded_block_total_sats"]) != int(postgres_payload["rewarded_block_total_sats"]):
        mismatches.append(
            _mismatch(
                "rewarded_block_total_sats",
                sqlite_payload["rewarded_block_total_sats"],
                postgres_payload["rewarded_block_total_sats"],
                "SQLite rewarded block total does not match Postgres settlement block reward total.",
            )
        )

    sqlite_payouts_by_username = {
        str(row["username"]): int(row["amount_sats"]) for row in sqlite_payload["user_payouts"]
    }
    postgres_payouts_by_username = {
        str(row["username"]): int(row["amount_sats"]) for row in postgres_payload["user_payouts"]
    }
...
def _audit_row_from_comparison(
    ...
    *,
    ...
    sqlite_payload = comparison.get("sqlite_summary") or {}
    row = {
        "settlement_id": comparison["settlement_id"],
        "period_start": sqlite_payload.get("period_start"),
        "period_end": sqlite_payload.get("period_end"),
        "status": sqlite_payload.get("status"),
...
def audit_postgres_shadow_settlements(
    ...
    *,
    ...
    for settlement in settlements:
        ...
        rows.append(_audit_row_from_comparison(comparison, include_details=include_details))
