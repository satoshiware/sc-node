...
set -euo pipefail
# =====================================================================================
# SC Node Stratum Setup (Bitcoin + AZCoin): Proxy, V1 to V2 Translator, Logger
#
# Details:
...
#       → "127.0.0.1:9093" # Bitcoin
#       → "127.0.0.1:9096" # AZCoin
#
# Ports:
#   - Bitcoin:  3333
#   - AZCoin:   3336
...
if ! id "stratum" &>/dev/null; then
    ...
    groupadd --system stratum 2>/dev/null || true
    useradd --system --gid stratum --shell /usr/sbin/nologin --comment "stratum Service" stratum 2>/dev/null || true
else
    log "System user 'stratum' already exists"
fi
...
# ===================== DIRECTORIES & FILES =====================
mkdir -p "$CONFIG_DIR" /var/log/stratum
chown -R stratum:stratum "$CONFIG_DIR" /var/log/stratum

touch /var/log/stratum/stratum-bitcoin.log
touch /var/log/stratum/stratum-azcoin.log
chown stratum:stratum /var/log/stratum/*.log
chmod 644 /var/log/stratum/*.log
...
chown root:root "$BIN_DIR/translator_sv2"
chmod 755 "$BIN_DIR/translator_sv2"
...
# ── Channel Mode ──
# Disabled for all miners to pass-through to the upstream
aggregate_channels = false

# ── Log Output ──
log_file = "/var/log/stratum/stratum-bitcoin.log"
...
# ── Log Output ──
log_file = "/var/log/stratum/stratum-azcoin.log"
...
# ── Setup HTTP Monitoring Server ──
# Enable API endpoints on a given port
monitoring_address = "127.0.0.1:9096"
monitoring_cache_refresh_secs = 15
...
User=stratum
Group=stratum
ExecStart=$BIN_DIR/translator_sv2 -c $CONFIG_DIR/stratum-bitcoin.toml
Restart=always
RestartSec=5
StandardOutput=append:/var/log/stratum/stratum-bitcoin-stdout.log
StandardError=append:/var/log/stratum/stratum-bitcoin-stderr.log
...
User=stratum
Group=stratum
ExecStart=$BIN_DIR/translator_sv2 -c $CONFIG_DIR/stratum-azcoin.toml
Restart=always
RestartSec=5
StandardOutput=append:/var/log/stratum/stratum-azcoin-stdout.log
StandardError=append:/var/log/stratum/stratum-azcoin-stderr.log
...
systemctl enable --now stratum-bitcoin stratum-azcoin 2>/dev/null || true
...
cat > /etc/logrotate.d/stratum-bitcoin << 'EOF'
/var/log/stratum/stratum-bitcoin.log
/var/log/stratum/stratum-bitcoin-*.log {
    daily
    rotate 14
    size 100M
...
EOF
chmod 644 /etc/logrotate.d/stratum-bitcoin

cat > /etc/logrotate.d/stratum-azcoin << 'EOF'
/var/log/stratum/stratum-azcoin.log
/var/log/stratum/stratum-azcoin-*.log {
    daily
    rotate 14
    size 100M
...
EOF
chmod 644 /etc/logrotate.d/stratum-azcoin
...
cat > "${README_FILE}" << EOF
Stratum (Bitcoin + AZCoin) Proxy, V1 to V2 Translator, Logger
...
Log Files:
  - Bitcoin: /var/log/stratum/stratum-bitcoin.log
             /var/log/stratum/stratum-bitcoin-stdout.log
             /var/log/stratum/stratum-bitcoin-stderr.log
  - AZCoin:  /var/log/stratum/stratum-azcoin.log
             /var/log/stratum/stratum-azcoin-stdout.log
             /var/log/stratum/stratum-azcoin-stderr.log
  - Logrotate (bitcoin): /etc/logrotate.d/stratum-bitcoin
  - Logrotate ( azcoin): /etc/logrotate.d/stratum-azcoin
